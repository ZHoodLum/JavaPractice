...
可是这个时候的错误提示信息并不完善，与之前的非空错误提示相比，因为现在知识给出了一个代号而已，再定义约束的时候没有为约束指定一个名字，所以系统默认分配了，而且约束的名字建议格式：“ 约束类型——字段  ”，例如：“UK_email”,指定约束名称使用” constraint “完成。
```
